...
export const HDFSInfo = {
    ...
    toJSON(message) {
        ...
        if (message.decommissioningNodes) {
            obj.decommissioningNodes = message.decommissioningNodes.map((e) => e ? HDFSNodeInfo.toJSON(e) : undefined);
        }
        else {
            obj.decommissioningNodes = [];
        }
        if (message.decommissionedNodes) {
            obj.decommissionedNodes = message.decommissionedNodes.map((e) => e ? HDFSNodeInfo.toJSON(e) : undefined);
        }
        else {
...
const baseZookeeperInfo = { alive: false };
...
const baseOozieInfo = { alive: false };
...
const baseLivyInfo = { alive: false };
...
const baseReportRequest = { cid: '', topologyRevision: 0 };
...
export const DataprocManagerServiceService = {
    ...
    report: {
        ...
        requestSerialize: (value) => Buffer.from(ReportRequest.encode(value).finish()),
        requestDeserialize: (value) => ReportRequest.decode(value),
        responseSerialize: (value) => Buffer.from(ReportReply.encode(value).finish()),
        responseDeserialize: (value) => ReportReply.decode(value),
    },
};
export const DataprocManagerServiceClient = makeGenericClientConstructor(DataprocManagerServiceService, 'yandex.cloud.dataproc.manager.v1.DataprocManagerService');
var globalThis = (() => {
